...
public class TrainerBiz implements Biz<String, TrainerVO>{
	...
	// 페이지, 정렬
	public List<TrainerVO> getbypage(int pageNo, int amount, String orderBy, String status) throws Exception{
		
		int offset = (pageNo - 1) * amount;
		List<TrainerVO> list = dao.selectbypage(pageNo, amount, orderBy, offset, status);
	...
	public List<TrainerVO> getforpreference(String gender, String zip, String[] workdays, String[] majors) throws Exception{
		List<TrainerVO> list = dao.selectforpreference(zip);
		for (TrainerVO v : list) {
